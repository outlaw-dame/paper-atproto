...
import type { AtUri, ContributionRole, ContributionScores, ThreadInterpolatorState, ContributorImpact, EntityImpact } from '../intelligence/interpolatorTypes.js';
import type { VerificationOutcome } from '../intelligence/verification/index.js';
import type { ConfidenceState, SummaryMode, InterpolatorWriteResult, WriterEntity } from '../intelligence/llmContracts.js';
export type ConversationSessionId = AtUri;
export type DeferredReason = 'outside_focused_branch' | 'collapsed_for_readability' | 'moderation_hidden' | 'unresolved_fetch_depth' | 'manual_collapse';
export type ConversationDirection = 'forming' | 'clarifying' | 'escalating' | 'stalled' | 'fragmenting';
export type ConversationalRole = 'anchor' | 'context_setter' | 'new_information' | 'clarification' | 'agreement' | 'disagreement' | 'question' | 'evidence' | 'escalation' | 'repetition' | 'tangent' | 'op_continuation' | 'unknown';
