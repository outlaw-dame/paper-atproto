export function projectComposerContext(params) {
    ...
    const selectedCommentTexts = parent
        ...
        : [];
    const threadSummary = session.interpretation.writerResult?.collapsedSummary
        ?? session.interpretation.interpolator?.summaryText;
    const directParentSummary = parent
        ? summarizeParentForComposer(parent.text)
        : undefined;
    const replyContextSummary = buildReplyContextSummary(siblingTexts, selectedCommentTexts);
    return {
        ...
        summaries: {
            ...
            conversationHeatSummary: `Tone: ${session.interpretation.threadState?.dominantTone ?? 'forming'}, phase: ${session.interpretation.threadState?.conversationPhase ?? 'active'}, heat: ${Math.round((session.trajectory.heatLevel ?? 0) * 100)}%`,
        },
        threadState: {
...
function collectAncestorTexts(session, startUri) {
    const texts = [];
...
function collectBranchTexts(session, startUri) {
...
function summarizeParentForComposer(text) {
    const normalized = text.trim().replace(/\s+/g, ' ');
    return normalized.length <= 220 ? normalized : `${normalized.slice(0, 217)}...`;
}
function buildReplyContextSummary(siblingReplyTexts, selectedCommentTexts) {
    const total = siblingReplyTexts.length + selectedCommentTexts.length;
    if (total === 0)
        return undefined;
    if (total <= 2)
        return 'Only light nearby reply activity.';
    if (total <= 6)
        return 'There is moderate nearby reply activity.';
    return 'There is active nearby reply traffic, so clarity and tone matter more.';
}
//# sourceMappingURL=composerProjection.js.map