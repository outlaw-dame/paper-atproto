...
export class SportsFeedService {
    ...
    isSportsPost(post) {
        ...
        if (/\b(live|game|score|final|q\d|inning|period|halftime|overtime|championship|playoffs)\b/i.test(text)) {
            return true;
        }
        // Check for sports terminology
        if (/\b(touchdown|field goal|home run|goal|assist|rebound|basket|wicket|birdie|ace|slam|serve)\b/i.test(text)) {
            return true;
        }
     ...
     */
    scoreSportsPost(post, liveGames) {
        let score = 0;
        const text = this.getPostText(post);
        ...
        if (liveGames) {
            for (const game of liveGames) {
                if (text.includes(game.homeTeam.name) ||
     ...
     */
    filterPosts(posts, filter, liveGames) {
        return posts
            .filter((post) => {
            ...
            if (filter.leagues && filter.leagues.length > 0) {
                const matchesLeague = filter.leagues.some((league) => text.includes(league.toLowerCase()));
                if (!matchesLeague) {
                    return false;
                }
            }
            ...
            if (filter.sports && filter.sports.length > 0) {
                const matchesSport = filter.sports.some((sport) => text.includes(sport.toLowerCase()));
                if (!matchesSport) {
                    return false;
                }
            }
            ...
            if (filter.sortBy === 'engagement') {
                const engagementA = (a.likeCount || 0) +
                    (a.replyCount || 0) * 2 +
                    (a.repostCount || 0) * 3;
                const engagementB = (b.likeCount || 0) +
                    (b.replyCount || 0) * 2 +
                    (b.repostCount || 0) * 3;
                return engagementB - engagementA;
            }
            else if (filter.sortBy === 'recency') {
                return (new Date(this.getPostCreatedAt(b)).getTime() -
                    new Date(this.getPostCreatedAt(a)).getTime());
            }
            else if (filter.sortBy === 'official-first') {
                const officialDelta = Number(!!(getLeagueByDid(b.author?.did) || getTeamByDid(b.author?.did))) -
                    Number(!!(getLeagueByDid(a.author?.did) || getTeamByDid(a.author?.did)));
                if (officialDelta !== 0)
                    return officialDelta;
                return (this.scoreSportsPost(b, liveGames) -
     ...
     */
    generateFeedSkeleton(posts, filter, liveGames) {
...
//# sourceMappingURL=sportsFeed.js.map