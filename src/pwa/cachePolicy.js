...
const CACHEABLE_SAME_ORIGIN_PATTERNS = [
...
export function getCacheStrategyForUrl(input) {
    let url;
    try {
    ...
    const full = url.href;
    if (NEVER_CACHE_PATTERNS.some((p) => p.test(full)))
        return 'network-only';
    ...
    if (AVATAR_ORIGINS.has(url.origin))
        return 'stale-while-revalidate';
    return 'network-only';
}
export function isCacheableReadRequest(url) {
