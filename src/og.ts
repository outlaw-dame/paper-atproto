...
const cache = new Map<string, Promise<OGMetadata | null>>();
// Circuit breaker per proxy target to avoid hammering blocked/rate-limited providers.
const proxyDisabledUntil = new Map<ProxyTarget['id'], number>();

export const fetchOGData = (url: string): Promise<OGMetadata | null> => {
...
function buildMetaOnlyHtml(rawHtml: string): string {
...
async function _fetchOGData(url: string): Promise<OGMetadata | null> {
  try {
    ...
    if (authorProfileUrl) metadata.authorProfileUrl = authorProfileUrl;
...
function normalizeOGUrl(rawUrl: string): string {
  try {
...
async function fetchHtmlThroughProxy(url: string): Promise<string | null> {
  const now = Date.now();
...
function looksLikeProxyBlockedPage(html: string): boolean {
  const lower = html.toLowerCase();
  return (
    lower.includes('corsproxy.io/_next/static/media/') ||
