...
function notifyAuthExpired(message) {
    ...
    window.dispatchEvent(new CustomEvent(ATP_AUTH_EXPIRED_EVENT, {
        detail: { message },
    }));
}
function recordAuthFailureDebug(kind, status, message) {
    if (typeof window === 'undefined')
        return;
    ...
    try {
        sessionStorage.setItem('glimpse:oauth:last-auth-failure', JSON.stringify({
            timestamp: Date.now(),
            kind,
            status,
            message: safeMessage,
 ...
 */
export async function atpCall(fn, opts = {}) {
    ...
    try {
        return await withRetry((_attempt, _signal) => fn(composedSignal), { ...retryOpts, signal: composedSignal });
    }
    catch (raw) {
 ...
 */
export async function atpMutate(fn, opts = {}) {
    try {
