...
const CONSTRUCTIVE_SIGNAL_PATTERNS = [
    {
...
const CONTENTIOUS_TOPIC_RE = /\b(abortion|gun\s+control|immigration|trans\s+rights|critical\s+race|election\s+(fraud|integrity)|vaccine|antivaxx?|climate\s+(change|denial)|roe\s+v\s+wade|blm|antifa|proud\s+boys|qanon)\b/i;
...
function analyzeParent(parentText) {
    const heatSignals = [];
...
function uniqSignals(values) {
    return Array.from(new Set(values.filter(Boolean)));
}
function strongestPositiveModelSignal(tone) {
    const { hostile, supportive, constructive, positive, } = tone.scores;
    if (supportive >= SUPPORTIVE_MODEL_THRESHOLD
        && supportive >= hostile + MODEL_SCORE_MARGIN) {
        return 'supportive';
    }
    if (constructive >= CONSTRUCTIVE_MODEL_THRESHOLD
        && constructive >= hostile + MODEL_SCORE_MARGIN) {
        return 'constructive';
    }
    if (positive >= POSITIVE_MODEL_THRESHOLD
        && positive >= hostile + MODEL_SCORE_MARGIN) {
        return 'positive';
    }
    return null;
}
function toneLooksHostile(tone, isReplyContext) {
...
function mergeToneModelIntoSentiment(base, text, tone) {
    ...
    if (uniquePositiveSignals.join() === base.signals.join()
...
export function analyzeSentiment(text, options = {}) {
    const trimmed = text.trim();
    const { parentText, parentReplyCount = 0, parentThreadCount = 0, threadTexts = [], commentTexts = [], totalCommentCount = 0, } = options;
    // Check for mental health crisis language immediately
    const mentalHealthCrisis = detectMentalHealthCrisis(trimmed);
    const isMediumActivityThread = parentReplyCount >= 25 ||
        parentThreadCount >= 10 ||
        totalCommentCount >= 25;
    const isLargeActivityThread = parentReplyCount >= 100 ||
...
export async function analyzeSentimentWithModel(text, options = {}, classifyTone) {
