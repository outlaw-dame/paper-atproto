 ...
 */
export async function fetchTrendingTopics(agent) {
    if (trendingCacheEntry && Date.now() < trendingCacheEntry.expiresAt) {
        return trendingCacheEntry.value;
    }
    try {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const res = await atpCall((s) => agent.app.bsky.unspecced.getTrendingTopics({ limit: 25 }), {
            timeoutMs: 8_000,
            maxAttempts: 1,
        });
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const raw = res.data?.topics ?? res.data?.suggestions ?? [];
 ...
 */
export async function fetchHashtagVolume(agent, tag) {
    const key = tag.toLowerCase();
    const cached = volumeCache.get(key);
    if (cached && Date.now() < cached.expiresAt)
        return cached.value;
    try {
        const res = await atpCall(() => agent.app.bsky.feed.searchPosts({ q: `#${key}`, limit: 1 }), { timeoutMs: 5_000, maxAttempts: 1 });
        // hitsTotal is present in the Bluesky API response but not yet in all SDK type stubs
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const hits = res.data.hitsTotal ?? res.data.posts.length;
 ...
 */
async function computeRelevanceScores(postText, tags) {
 ...
 */
export async function getHashtagInsights(agent, tags, postText) {
    if (!tags.length)
        return [];
    ...
    return tags.map((tag, i) => {
        ...
        const score = Math.min(100, Math.round(volumeScore * 0.5 + relevanceScore * 0.35 + (isTrending ? 15 : 0)));
        const label = isTrending ? 'Trending' : score > 65 ? 'Popular' : 'Active';
        return { tag, score, volumeScore, relevanceScore, isTrending, label };
    });
}
//# sourceMappingURL=hashtagInsights.js.map