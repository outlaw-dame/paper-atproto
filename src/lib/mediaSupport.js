...
function strongestSupport(levels) {
    return levels.reduce((best, current) => (SUPPORT_ORDER[current] > SUPPORT_ORDER[best] ? current : best), '');
}
function buildSupportProbe(video, key, label, mimeTypes) {
...
function emptySupport(key, label, mimeType) {
    return {
        key,
        label,
        mimeType,
...
export function getVideoPlaybackCapabilities() {
...
export function detectVideoSourceKind(url) {
    ...
    try {
        ...
        if (pathname.endsWith('.m3u8') || pathname.includes('/playlist'))
            return 'hls';
        if (pathname.endsWith('.mp4') || pathname.endsWith('.m4v') || pathname.endsWith('.mov'))
            return 'mp4';
        ...
        if (normalized.includes('.m3u8'))
            return 'hls';
        if (normalized.includes('.mp4') || normalized.includes('.m4v') || normalized.includes('.mov'))
            return 'mp4';
        if (normalized.includes('.webm'))
            return 'webm';
    }
    if (normalized.includes('video.bsky.app'))
        return 'hls';
    return 'unknown';
}
export function getLikelySourceSupport(capabilities, sourceKind) {
    if (sourceKind === 'hls')
        return capabilities.hls.supported;
    if (sourceKind === 'mp4')
        return capabilities.mp4.supported;
    if (sourceKind === 'webm')
        return capabilities.webm.supported;
    return null;
}
export function describeSupportLevel(level) {
    if (level === 'probably')
        return 'Yes';
    if (level === 'maybe')
        return 'Maybe';
...
export function describeSourceKind(sourceKind) {
    if (sourceKind === 'hls')
        return 'HLS stream';
    if (sourceKind === 'mp4')
        return 'MP4 file';
...
export function getLikelyUnsupportedReason(capabilities, sourceKind) {
...
//# sourceMappingURL=mediaSupport.js.map