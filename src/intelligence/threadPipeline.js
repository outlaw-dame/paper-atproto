...
import { InMemoryVerificationCache } from './verification/cache.js';
...
function toContributionScores(score) {
...
function shouldVerifyPost(post, score) {
    if (!post.text.trim())
        return false;
    if (score.finalInfluenceScore >= 0.60)
        return true;
    if (score.clarificationValue >= 0.55)
        return true;
    if (score.sourceSupport >= 0.50)
        return true;
    if (score.role === 'rule_source' || score.role === 'source_bringer')
        return true;
    if ((post.embeds?.length ?? 0) > 0)
        return true;
...
function selectVerificationCandidates(posts, scores, limit) {
    return [...posts]
        .filter(p => {
...
function toVerificationRequest(post, signal) {
    const req = { postUri: post.uri, text: post.text };
    if (post.indexedAt !== undefined)
        req.createdAt = post.indexedAt;
    if (post.facets?.length)
        req.facets = post.facets;
    if (post.embeds?.length)
        req.embeds = post.embeds;
    if (post.media?.length)
        req.media = post.media;
    if (post.entities?.length)
        req.entities = post.entities;
    if (signal !== undefined)
        req.signal = signal;
    return req;
}
// ─── runVerifiedThreadPipeline ────────────────────────────────────────────
export async function runVerifiedThreadPipeline(options) {
    const { input, providers, signal, verificationLimit = 8, verificationConcurrency = 3, } = options;
    ...
    for (const [uri, score] of Object.entries(interpolator.replyScores)) {
        scores[uri] = toContributionScores(score);
    }
    ...
    const verificationByPost = {};
    await mapWithConcurrency(candidates, async (post) => {
        try {
            const request = toVerificationRequest(post, signal);
            const outcome = await withRetry(() => verifyEvidence(request, providers, undefined, { cache }), { retries: 2, ...(signal !== undefined ? { signal } : {}) });
    ...
    let rootVerification = null;
    if (input.rootPost !== undefined && input.rootPost.text.trim()) {
        try {
            const rootRequest = toVerificationRequest(input.rootPost, signal);
            rootVerification = await withRetry(() => verifyEvidence(rootRequest, providers, undefined, { cache }), { retries: 2, ...(signal !== undefined ? { signal } : {}) });
        }
        catch {
            // Root verification is best-effort
        }
    }
    const didMeaningfullyChange = options.previous == null ||
...
//# sourceMappingURL=threadPipeline.js.map