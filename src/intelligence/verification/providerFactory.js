...
import { HttpClaimExtractorProvider, HttpFactCheckProvider, HttpGroundingProvider, HttpMediaVerificationProvider, } from './httpProviders.js';
let cachedProviders = null;
function readBaseUrl() {
    try {
        // import.meta.env is Vite-specific; guard for non-browser test environments
        const value = typeof import.meta !== 'undefined'
            ? import.meta.env?.VITE_GLYMPSE_VERIFY_BASE_URL
            : undefined;
...
function readTimeoutMs() {
    try {
        const raw = typeof import.meta !== 'undefined'
            ? import.meta.env?.VITE_GLYMPSE_VERIFY_TIMEOUT_MS
            : undefined;
        const n = Number(raw);
 ...
 * is not configured.
 */
export function createVerificationProviders() {
 ...
 */
export function resetVerificationProviders() {
