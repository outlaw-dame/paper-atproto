import { InMemoryVerificationCache, verificationCacheKey } from './cache.js';
import { buildReasons, clamp01, computeSourceTypeFromUrls, entityGroundingScore, sanitizeVerificationRequest, sourceTypeQuality, specificityScore, } from './utils.js';
import { HeuristicClaimExtractorProvider, NoopFactCheckProvider, NoopGroundingProvider, NoopMediaVerificationProvider, } from './noopProviders.js';
function quoteFidelityScore(input) {
    const lower = input.text.toLowerCase();
    if (/["\u201C\u201D]/.test(input.text))
        return 0.7;
    if (/\bthe rule says\b|\bthe report says\b|\baccording to\b/.test(lower))
        return 0.6;
    return 0;
}
function correctionValueScore(input) {
    const lower = input.text.toLowerCase();
    let score = 0;
    if (/\bactually\b|\bto clarify\b|\bfor context\b|\bin fact\b/.test(lower))
        score += 0.35;
    if (/\bmisleading\b|\bout of context\b|\bincomplete\b|\bthe rule says\b/.test(lower))
        score += 0.25;
    if (/\bnot\b|\bincorrect\b|\bdoesn'?t\b|\bwasn'?t\b/.test(lower))
        score += 0.1;
    return clamp01(score);
}
function contextValueScore(input, grounding) {
    let score = 0;
    if (grounding?.summary)
        score += 0.35;
    if ((grounding?.sources.length ?? 0) >= 1)
        score += 0.2;
    if ((grounding?.sources.length ?? 0) >= 2)
        score += 0.15;
...
function sourcePresenceScore(input, grounding) {
...
function aggregateSourceQuality(grounding, fallbackSourceType) {
...
function computeFactualContributionScore(input) {
    return clamp01(0.18 * input.checkability +
...
function computeFactualConfidence(input) {
    return clamp01(0.22 * input.sourcePresence +
...
function resolveFactualState(input) {
    if (input.factCheckMatched)
        return 'known-fact-check-match';
    if (input.mediaContextWarning)
        return 'media-context-warning';
    if (input.factualContributionScore >= 0.70 && input.factualConfidence >= 0.78)
        return 'well-supported';
    if (input.correctionValue >= 0.60 && input.factualConfidence >= 0.68)
        return 'source-backed-clarification';
    if (input.contradictionLevel >= 0.45)
        return 'contested';
    if (input.factualContributionScore >= 0.55 && input.factualConfidence >= 0.58)
        return 'partially-supported';
    if (input.factualContributionScore > 0.20)
        return 'unsupported-so-far';
    return 'none';
}
export async function verifyEvidence(requestInput, providers, options, context) {
    ...
    const [factCheckResult, groundingResult, mediaResult] = await Promise.all([
        options?.useFactCheck === false
            ...
            : factCheckProvider.lookup({ request, claims, ...(request.signal !== undefined ? { signal: request.signal } : {}) }).catch((error) => {
                providerFailures.push(`factCheck:${error.message}`);
                return null;
            }),
        options?.useGrounding === false
            ...
            : groundingProvider.ground({ request, claims, ...(request.signal !== undefined ? { signal: request.signal } : {}) }).catch((error) => {
                providerFailures.push(`grounding:${error.message}`);
                return null;
            }),
        options?.useMediaVerification === false || !(request.media?.length)
            ...
            : mediaProvider.inspect({ request, claims, ...(request.signal !== undefined ? { signal: request.signal } : {}) }).catch((error) => {
                providerFailures.push(`media:${error.message}`);
                return null;
            }),
    ]);
    ...
    if (factCheckResult?.matched)
        reasons.unshift('known-fact-check-match');
    const outcome = {
        request,
        extractedClaims,
...
//# sourceMappingURL=verifyEvidence.js.map