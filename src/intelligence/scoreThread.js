...
import { linkAndMatchEntities } from './entityLinking.js';
// ─── Evidence extraction ──────────────────────────────────────────────────
...
function extractEvidenceSignals(text, facets, embed) {
    const signals = [];
...
function computeFactualContribution(signals) {
    if (signals.length === 0)
        return 0;
    let score = 0;
    for (const s of signals) {
        switch (s.kind) {
            case 'citation':
                score += 0.30 * s.confidence;
                break;
            case 'data_point':
                score += 0.25 * s.confidence;
                break;
            case 'firsthand':
                score += 0.20 * s.confidence;
                break;
            case 'counterexample':
                score += 0.15 * s.confidence;
                break;
            case 'speculation':
                score -= 0.08;
                break;
...
function deriveLocalFactCheckMatch(text, rootText, allCitedUrls, signals) {
    ...
    const rootWords = new Set(rootText.toLowerCase().split(/\s+/).filter(w => w.length > 5));
    ...
    if (overlapRatio > 0.35 && citations.length > 0)
        return { match: true, confidence: 0.40 };
    return { match: false, confidence: 0 };
}
// ─── ContributionRole decision tree ──────────────────────────────────────
function assignRole(text, threadTexts, signals, likeCount, factualContribution) {
    const lower = text.toLowerCase();
    ...
    if (hasDisagreement && hasCitation)
        return 'useful_counterpoint';
    if (hasClarification || (hasQuestion && !hasDisagreement))
        return 'clarifying';
    if (hasDisagreement && (hasData || hasFirsthand))
        return 'useful_counterpoint';
    if (hasDisagreement)
        return 'provocative';
    if (hasCitation || hasData || hasFirsthand)
        return 'new_information';
    if (hasAgreement && words < 8)
        return 'repetitive';
    if (likeCount > 20 && factualContribution > 0.25)
        return 'story_worthy';
    if (words > 25)
...
// ─── Usefulness score ─────────────────────────────────────────────────────
function computeUsefulnessScore(role, factualContribution, likeCount, words) {
    const base = {
...
// ─── scoreReply ───────────────────────────────────────────────────────────
export function scoreReply(reply, rootText, allReplies, entityCatalog, allCitedUrls) {
    ...
    const { match: knownFactCheckMatch, confidence: factCheckMatchConfidence } = deriveLocalFactCheckMatch(text, rootText, allCitedUrls, evidenceSignals);
    return {
        uri: reply.uri,
        role,
        usefulnessScore,
...
export function scoreAllReplies(rootText, replies) {
...
export function computeContributorImpacts(replies, scores) {
    const byDid = new Map();
    for (const r of replies) {
        if (!r.authorDid)
            continue;
    ...
    for (const [did, { ss, handle }] of byDid) {
        ...
        const dominantRole = (Object.entries(counts).sort(([, a], [, b]) => b - a)[0]?.[0] ?? 'unknown');
        impacts.push({
            did,
...
//# sourceMappingURL=scoreThread.js.map