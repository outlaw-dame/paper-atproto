 ...
 */
export function computeInterpretiveConfidence(i) {
    return Math.max(0, Math.min(1, 0.25 * i.themeConfidence +
 ...
 */
export function computeSurfaceConfidence(state) {
    ...
    return Math.max(0, Math.min(1, 0.40 * hasRootText +
        0.35 * replySignal +
        0.25 * lowRepetition));
}
// ─── Entity confidence ────────────────────────────────────────────────────
 ...
 */
export function computeEntityConfidence(state) {
    if (state.entityLandscape.length === 0)
        return 0;
    const topEntities = state.entityLandscape.slice(0, 5);
    const avgMatch = topEntities.reduce((sum, e) => sum + (e.matchConfidence ?? 0.4), 0) / topEntities.length;
 ...
 */
export function computeConfidenceState(state, _scores) {
    const surfaceConfidence = computeSurfaceConfidence(state);
    const entityConfidence = computeEntityConfidence(state);
    const replyCount = Object.keys(state.replyScores).length;
    const highImpactContributors = state.topContributors.filter(c => c.avgUsefulnessScore >= 0.60).length;
