...
function heuristicConstructivePrior(heuristics) {
    if (heuristics.constructiveSignals.length === 0)
        return 0;
    return clamp01(0.56 + ((heuristics.constructiveSignals.length - 1) * 0.12));
}
function heuristicSupportivePrior(heuristics) {
    if (heuristics.supportiveReplySignals.length === 0)
        return 0;
    return clamp01(0.6 + ((heuristics.supportiveReplySignals.length - 1) * 0.12));
}
function heuristicClarifyingPrior(heuristics) {
...
function heuristicHostilityPrior(heuristics) {
...
function heuristicDismissivePrior(heuristics) {
    const joined = heuristics.signals.join(' ').toLowerCase();
...
function heuristicEscalationPrior(heuristics) {
...
function emotionScore(ml, label) {
    const hit = ml.emotions?.find((emotion) => emotion.label === label);
    return clamp01(hit?.score ?? 0);
}
function targetedNegativityScore(ml) {
    const targeted = ml.targetedTone;
    if (!targeted)
        return 0;
    if (targeted.label === 'strongly_negative')
        return clamp01(targeted.confidence);
    if (targeted.label === 'negative')
        return clamp01(targeted.confidence * 0.82);
    if (targeted.label === 'negative_or_neutral')
        return clamp01(targeted.confidence * 0.45);
    return 0;
}
function sentimentPositiveScore(ml) {
    if (!ml.sentiment)
        return 0;
    if (ml.sentiment.label !== 'positive')
        return 0;
    return clamp01(ml.sentiment.confidence);
}
function sentimentNegativeScore(ml) {
    if (!ml.sentiment)
        return 0;
    if (ml.sentiment.label !== 'negative')
        return 0;
    return clamp01(ml.sentiment.confidence);
}
function qualityScore(ml, heuristicsValue, key) {
    return clamp01(Math.max(heuristicsValue, ml.conversationQuality?.[key] ?? 0));
}
export function createEmptyComposerGuidanceScores() {
    return {
        positiveSignal: 0,
        negativeSignal: 0,
        supportiveness: 0,
...
export function createEmptyComposerGuidanceResult(mode) {
    return {
        mode,
        level: 'ok',
        heuristics: {
...
export function hasVisibleComposerGuidance(result) {
    return result.level !== 'ok' || result.heuristics.parentSignals.length > 0;
}
export function computeComposerGuidanceScores(heuristics, ml, abuseScore) {
    ...
    const positiveSignal = clamp01((0.3 * supportiveness)
        + (0.25 * constructiveness)
        + (0.2 * clarifying)
        + (0.1 * trust)
        + (0.1 * optimism)
        + (0.05 * sentimentPositive));
    const negativeSignal = clamp01((0.25 * hostility)
        + (0.2 * dismissiveness)
        + (0.15 * escalation)
        + (0.15 * toxicity)
        + (0.15 * anger)
...
export function deriveComposerGuidanceState(heuristics, ml, abuseScore) {
    const scores = computeComposerGuidanceScores(heuristics, ml, abuseScore);
    if (heuristics.level === 'alert')
        return 'alert';
    if (heuristics.hasMentalHealthCrisis)
        return 'alert';
    if (abuseScore && abuseScore.score >= ABUSE_WARNING_THRESHOLD)
        return 'warning';
    if (scores.negativeSignal >= WARNING_NEGATIVE_THRESHOLD
    ...
    if (heuristics.level === 'positive'
        || (scores.positiveSignal >= POSITIVE_SIGNAL_THRESHOLD && scores.negativeSignal < LOW_NEGATIVE_THRESHOLD)) {
        return 'positive';
    }
    return 'neutral';
}
export function uiStateToLevel(state) {
    if (state === 'neutral')
        return 'ok';
    return state;
}
export function normalizeToolsUsed(toolsUsed) {
    return toolsUsed.filter((tool) => (tool === 'heuristic'
        || tool === 'zero-shot-tone'
        || tool === 'abuse-score'
...
//# sourceMappingURL=guidanceScoring.js.map