function uniqTrimmed(values, limit) {
    return Array.from(new Set(values
        .map((value) => value?.trim())
        .filter((value) => Boolean(value)))).slice(0, limit);
}
function buildTargetText(context) {
...
export function buildPostComposerContext(draftText) {
    return {
        mode: 'post',
        draftText,
    };
}
export function buildReplyComposerContext(input) {
...
export function buildHostedThreadComposerContext(input) {
...
export function toAnalyzeOptions(context) {
