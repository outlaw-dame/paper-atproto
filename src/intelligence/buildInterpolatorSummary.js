...
function topicPhrase(rootText) {
    ...
    if (base.length < clean.length && base.length <= 90)
        return base + '…';
    if (base.length > 90) {
...
function synthesiseSummary(rootText, entityLandscape, topContributors, clarificationsAdded, newAnglesAdded, sourceSupportPresent, factualSignalPresent, heatLevel, repetitionLevel, totalReplies, topEntitySentiment) {
    if (totalReplies === 0)
        return '';
...
export function buildInterpolatorSummary(rootText, replies, scores) {
    const salientClaims = [rootText.slice(0, 120)];
    const salientContributors = [];
    const clarificationsAdded = [];
    const newAnglesAdded = [];
    ...
    const sorted = [...replies].sort((a, b) => (scores[b.uri]?.usefulnessScore ?? 0) - (scores[a.uri]?.usefulnessScore ?? 0));
    for (const reply of sorted) {
        ...
        if (score.evidenceSignals.some(s => s.kind !== 'speculation'))
    ...
    const summaryText = synthesiseSummary(rootText, entityLandscape, topContributors, clarificationsAdded, newAnglesAdded, sourceSupportPresent, factualSignalPresent, heatLevel, repetitionLevel, sorted.length, topEntitySentiment);
    return {
        summaryText,
        salientClaims,
        salientContributors,
...
//# sourceMappingURL=buildInterpolatorSummary.js.map