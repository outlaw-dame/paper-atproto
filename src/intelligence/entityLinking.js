...
function scoreEntitySentiment(text, entityLabel) {
...
function stableEntityId(kind, normalized) {
...
function applyConceptCanonicalization(normalized) {
...
function classifyProperNounEntity(label) {
    if (label.includes(' '))
        return 'org';
    return 'person';
}
...
function extractEntityCandidates(text, facets) {
...
function findBestCatalogMatch(candidate, catalog) {
    let best = null;
    const candidateTokens = tokenSet(candidate.normalized);
    for (const entry of catalog.values()) {
        if (entry.entityKind !== candidate.entityKind)
            continue;
        if (entry.aliases.has(candidate.normalized) || entry.normalizedLabel === candidate.normalized) {
            return { entry, confidence: 0.99 };
        }
        const lexical = Math.max(diceSimilarity(candidate.normalized, entry.normalizedLabel), ...[...entry.aliases].map(alias => diceSimilarity(candidate.normalized, alias)));
...
export function linkAndMatchEntities(text, facets, catalog) {
...
export function summarizeStoryEntities(texts) {
...
//# sourceMappingURL=entityLinking.js.map