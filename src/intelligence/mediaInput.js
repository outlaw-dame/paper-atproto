...
import { computeMultimodalScore } from './routing.js';
// ─── detectMediaSignals ───────────────────────────────────────────────────
// Compute multimodal routing signals from the resolved thread.
export function detectMediaSignals(root, replies, scores) {
    const rootHasMedia = (root.embed?.kind === 'images' || root.embed?.kind === 'recordWithMedia') ? 1 : 0;
    const replyMediaCount = replies.filter(r => r.embed?.kind === 'images' || r.embed?.kind === 'recordWithMedia').length;
...
export function selectMediaForAnalysis(threadId, root, replies, scores) {
    const requests = [];
...
export function mergeMediaResults(results) {
    return results
        ...
        if (r.extractedText)
            finding.extractedText = r.extractedText;
        if (r.cautionFlags.length > 0)
            finding.cautionFlags = r.cautionFlags;
        return finding;
    });
}
