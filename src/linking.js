 ...
 */
export const linkToWikidata = async (entityText) => {
    try {
 ...
 */
export const processTextEntities = async (text) => {
    const entities = await extractEntities(text);
    // Link each entity to Wikidata in parallel
    const linkedEntities = await Promise.all(entities.map(async (entity) => {
