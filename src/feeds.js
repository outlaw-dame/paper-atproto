...
export class FeedService {
    getFirstChildByLocalName(parent, localName) {
    ...
    parsePodcast20Metadata(xmlContent) {
            ...
            for (let i = 0; i < items.length; i += 1) {
                ...
                if (valueEl) {
                    ...
                    for (let r = 0; r < recipients.length; r += 1) {
                        ...
                        if (recipientNode.localName?.toLowerCase() !== 'valuerecipient')
                            continue;
                        const record = {};
                ...
                const keys = [guid, link, enclosureUrl].filter((key) => Boolean(key));
    ...
    getPodcast20ForItem(item, metadataByKey) {
        const keyCandidates = [
            item?.enclosure?.url,
            item?.id,
            item?.guid,
     ...
     */
    async parseRdfXmlFeed(xmlContent) {
        ...
        for (let i = 0; i < itemElements.length; i++) {
            ...
            const getTextContent = (tag) => el.getElementsByTagName(tag)[0]?.textContent;
            items.push({
     ...
     */
    async addFeed(url, category = 'News') {
        try {
            ...
            const feedResult = await pg.query(`INSERT INTO feeds (url, title, description, type, category, last_synced_at)
         VALUES ($1, $2, $3, $4, $5, NOW())
         ON CONFLICT (url) DO UPDATE SET
           title = EXCLUDED.title,
           description = EXCLUDED.description,
           last_synced_at = NOW()
         RETURNING id`, [url, parsed.title, parsed.description, parsed.type, category]);
            ...
            for (const item of parsed.items) {
                ...
                await pg.query(`INSERT INTO feed_items (
     ...
     */
    async generateLocalFeed(type = 'rss') {
        const pg = paperDB.getPG();
        ...
        if (type === 'atom')
            return generateAtomFeed(feedData);
        if (type === 'json')
            return generateJsonFeed(feedData);
        if (type === 'jsonld')
            return this.generateJsonLdFeed(feedData);
        if (type === 'rdf')
            return this.generateRdfXmlFeed(feedData);
        return generateRssFeed(feedData);
    }
    /**
     * Generate JSON-LD format feed.
     */
    generateJsonLdFeed(feedData) {
        const jsonld = {
            ...
            itemListElement: feedData.items.map((item, idx) => ({
                '@type': 'ListItem',
                position: idx + 1,
                url: item.link,
                item: {
...
<rdf:RDF
  ...
  <channel rdf:about="${this.escapeXml(feedData.link)}">
    ...
    <items>
      <rdf:Seq>
${feedData.items.map((item) => `        <rdf:li rdf:resource="${this.escapeXml(item.link)}"/>`).join('\n')}
      </rdf:Seq>
    </items>
  </channel>
${items}
    ...
    async getFeedItems(feedId) {
        const pg = paperDB.getPG();
        const result = await pg.query('SELECT * FROM feed_items WHERE feed_id = $1 ORDER BY pub_date DESC LIMIT 50', [feedId]);
        return result.rows;
    }
    /**
    ...
    async getRecentFeedItems(limit = 20) {
        const pg = paperDB.getPG();
        const result = await pg.query(`SELECT fi.*, f.title AS feed_title, f.category AS feed_category, f.type AS feed_type
       FROM feed_items fi
