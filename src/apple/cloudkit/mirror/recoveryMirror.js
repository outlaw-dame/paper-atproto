...
// ─── Public API ───────────────────────────────────────────────────────────────
export async function mirrorDraftRecovery(userDid, draftId, plaintextPayload) {
    if (!ENABLED)
        return;
    try {
...
export async function restoreDraftRecovery(userDid, draftId) {
    if (!ENABLED)
        return null;
    try {
        const client = await getCloudKitClient();
...
async function deriveKey(userDid) {
    const salt = getOrCreateSalt();
    const enc = new TextEncoder();
    const keyMaterial = await crypto.subtle.importKey('raw', enc.encode(userDid), 'PBKDF2', false, ['deriveKey']);
    return crypto.subtle.deriveKey({ name: 'PBKDF2', salt: salt.buffer, iterations: PBKDF2_ITERATIONS, hash: 'SHA-256' }, keyMaterial, { name: ALGORITHM, length: KEY_BITS }, false, ['encrypt', 'decrypt']);
}
async function encrypt(plaintext, userDid) {
...
async function decrypt(encryptedPayload, iv, userDid) {
    const key = await deriveKey(userDid);
...
//# sourceMappingURL=recoveryMirror.js.map