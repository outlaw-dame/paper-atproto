...
export function detectAppleEnhancementAvailability() {
    ...
    try {
        ...
        const cloudKitJsAvailable = isCloudKitLoaded() || canLoadCloudKitScript(window.location.origin);
        const pwaInstalled = window.matchMedia('(display-mode: standalone)').matches ||
            window.matchMedia('(display-mode: minimal-ui)').matches ||
            navigator.standalone === true;
        const notificationsUsable = 'Notification' in window &&
            Notification.permission === 'granted' &&
            'serviceWorker' in navigator &&
            'PushManager' in window;
        const badgingUsable = 'setAppBadge' in navigator || 'clearAppBadge' in navigator;
        // WebKit detection: capability-first, UA as fallback only.
        const ua = navigator.userAgent;
        const likelyAppleWebKit = ('standalone' in navigator && !('chrome' in window)) ||
