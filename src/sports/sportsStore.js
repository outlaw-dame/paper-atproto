...
class SportsStore {
    ...
    isActuallyLive(game) {
        ...
        if (updated !== null && now - updated > 20 * 60 * 1000)
            return false;
        return true;
    }
    /**
     ...
     */
    async updateFromExternalAPI(gameId, apiProvider = 'mock') {
        try {
            if (apiProvider === 'mock') {
            ...
            const league = apiProvider === 'espn' ? 'nba' : apiProvider;
     ...
     */
    startPolling(gameId, apiProvider = 'mock') {
    ...
    async loadFromEspn(leagues = ['nba', 'nfl']) {
        await Promise.all(leagues.map((league) => this.syncEspnLeague(league)));
    }
    startEspnAutoRefresh(leagues = ['nba', 'nfl'], intervalMs = 45_000) {
     ...
     */
    getUpdateHistory(gameId, limit = 100) {
    ...
    mapEspnStatus(statusRaw, detailRaw) {
    ...
    mapEspnEventToLiveGame(event, league) {
     ...
     */
    mockFetchGameState(gameId, _provider) {
