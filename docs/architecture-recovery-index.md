...
11. [DONE] Coordinator runtime extraction around `sessionAssembler.ts`. Runtime integration completed: source-token guard decisions, coordinator context snapshot diagnostics, model-stage planner run/skip gating, media stage executor delegation, writer stage executor delegation, and premium stage executor delegation are integrated. `coordinatorMediaStageExecutor`, `coordinatorWriterStageExecutor`, and `coordinatorPremiumStageExecutor` now own plan/execute and validation/normalization respectively.

    Notes:
    - Writer delegation introduces defensive shape validation, empty-summary rejection, and bounded normalization (collapsedSummary ≤1.2k, expandedSummary ≤4k, whatChanged ≤8 entries, contributorBlurbs deduped).
    - Premium delegation runs with `retryPolicy.maxAttempts: 1` to avoid stacking on the existing inner retry inside `callPremiumDeepInterpolator`.

    Final collapse (item 7) split `hydrateConversationSession` into private stage helpers inside `sessionAssembler.ts`, all pure cut/paste with no behavior change:
    - **7a `hydrateConversationBaseline`** — thread fetch → verified pipeline → mental-health scan → quality/policy/continuity transforms → shadow supervisor → coordinator advisory log → source-token build. Returns `ready` / `thread_not_found` / `session_dropped`.
    - **7b `runConversationWriterPreflight`** — interpolator-disabled fast path, gate decision logging, no-meaningful-change skip fan-out.
    - **7c `runConversationMediaStage`** — media plan derivation, multimodal planner skip, media executor invocation, `hydrate.multimodal` timing, `multimodal_completed` shadow event.
    - **7d `runConversationWriterStage`** — post-media source guard, `buildThreadStateForWriter`, writerEntities store update, writer executor invocation, `hydrate.writer` timing, `writer_completed` shadow event.
    - **7e `runConversationPremiumStage`** — actorDid resolution, entitlements fetch with `hydrate.premium_entitlements` timing, deep interpolator gate, redaction + executor invocation with `retryPolicy.maxAttempts: 1`, `hydrate.premium_deep` timing, `premium_completed` shadow event, self-contained inner try/catch.

    The hydration caller now reads as: baseline → preflight → translation (inline) → media → writer → premium, with the outer try/catch retaining the writer-level error path and the `hydrate.total` finalizer.
